...
- Implemented in the menu-state increment for [#1972](https://github.com/noah-nuebling/mac-mouse-fix/issues/1972): opening the Buttons or Scrolling settings tab and hiding the menu-bar item no longer clear the persisted feature kill switches. Add Mode temporarily bypasses a feature kill switch for input capture without changing its saved value, while inactive-session and helper-lockdown checks remain authoritative. Focused persistence/source-policy and capture-policy suites pass, as do the signed Debug App/Helper build and deep signature verification. Quit/reopen and hidden/visible menu-bar transitions remain the manual runtime gate.
