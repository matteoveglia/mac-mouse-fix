...
- Planned as a separate input-ownership increment for [#1929](https://github.com/noah-nuebling/mac-mouse-fix/issues/1929), not an extension of keyboard modifier flags. The current helper observes only `flagsChanged`; ordinary key down/up events need an exclusive owner that swallows configured activator keys, ignores auto-repeat, and clears held state on key-up, tap disable, config replacement, helper lockdown, and shutdown. The first safe scope should accept only otherwise-unused F13–F24 virtual keys, which mouse utilities can emit without stealing normal typing. Add a distinct persisted key-activator precondition, a trigger-key capture UI, and a pure reducer before connecting it to `Modifiers`/`SwitchMaster`. MMF-generated shortcut events must carry provenance and bypass the activator path to prevent recursive gestures. Expanding to printable keys is deferred until the UI can make destructive key capture explicit and conflict-aware. Acceptance requires reducer coverage for down/repeat/up, replacement keys, tap timeout, stale release, synthetic bypass, and teardown; physical testing must cover typing, Razer-style remapping, drag/scroll activation, app switching, sleep/wake, and helper disable while held.
