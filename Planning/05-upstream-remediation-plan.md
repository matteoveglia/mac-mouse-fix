...
- Completed in the license-persistence increment for [#1939](https://github.com/noah-nuebling/mac-mouse-fix/issues/1939) and [#1975](https://github.com/noah-nuebling/mac-mouse-fix/issues/1975): secure-storage lookup now distinguishes a readable key, item-not-found, and a temporarily unavailable Keychain. Startup preserves an existing paid cache for both item-not-found and transient read failures; the user's explicit Deactivate action clears that cache directly. The local non-synchronizable fallback retains the complete dictionary after a readable-item update failure, refuses to replace unreadable storage with a partial dictionary, and remains authoritative on later launches so locally written activation data cannot be hidden by an older synchronized item. Pure policy tests cover found, missing-with-cache, missing-without-cache, unavailable-with-cache, and unavailable-without-cache decisions; the clean signed Debug App/Helper build, unsigned Release build, and deep signature verification pass. The user confirmed the activated license survived quit/reopen and reboot while offline, with Buttons and Scrolling still operating normally. An intentionally unavailable Keychain remains a diagnostic gate. Activation-server errors in #1941, license-management UX in #1957, and payment-provider failure in #1959 are separate server/product concerns and are not claimed fixed by this storage change.
- Completed in the independent zoom-speed increment for [#1956](https://github.com/noah-nuebling/mac-mouse-fix/issues/1956): config version 30 adds one global `Scroll.zoomSpeed` setting, exposed as a 50–200% slider with 10% steps and a 0.50x–2.00x display. Zoom uses a fixed medium baseline so ordinary vertical or horizontal speed changes do not change zoom sensitivity; the bounded multiplier is applied only to the final magnification delta, leaving ordinary scrolling and Smart Zoom unchanged. The 29→30 migration, malformed/non-finite slider handling, deterministic zoom tests, clean signed Debug App/Helper build, unsigned Release build, and macOS 27 `Tests` target build pass. The signed app’s manual zoom-speed behavior is the next test gate.
- [#1922](https://github.com/noah-nuebling/mac-mouse-fix/issues/1922) remains diagnostic-gated rather than “fixed”: the helper reconstructs and reposts synthetic scroll events, while the public `CGEventSource.pixelsPerLine` API controls pixel/line conversion rather than the macOS System Settings scroll-speed slider. The deprecated IOKit acceleration getter is not a safe unverified replacement. Capture a physical Razer reproduction and compare original versus reposted acceleration fields before changing scroll math or claiming parity.
...
- The first input-ownership increment for [#1929](https://github.com/noah-nuebling/mac-mouse-fix/issues/1929) is implemented separately from keyboard modifier flags. A dedicated intercepting key-down/key-up tap accepts only otherwise-unused F13–F20 virtual keys, swallows the configured key stream while held, suppresses repeats, and clears held state on key-up, tap disable, config replacement, helper lockdown, and shutdown. A distinct persisted `keyboardActivator` precondition is matched exactly and counted as a specificity dimension; Add Mode can capture F13–F20 alongside a mouse gesture. MMF-generated keyboard events carry provenance and bypass the activator path to prevent recursive gestures. The pure reducer and provenance suites cover malformed allow-lists, down/repeat/up, replacement, stale release, synthetic bypass, and idempotent reset; signed-build physical testing remains open for Razer-style remapping, drag/scroll activation, app switching, sleep/wake, and helper disable while held. Printable-key capture and per-row replacement/clear UI remain a separate follow-up so normal typing is never implicitly captured.
