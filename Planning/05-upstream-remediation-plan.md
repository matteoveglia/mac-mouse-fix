...
- Completed in the pointer-freeze increment: a failed tap enable restores the default global cursor-suppression interval, a failed timeout recovery explicitly unfreezes the pointer, and helper teardown restores the real cursor/removes the puppet cursor if disable interrupts a drag.
