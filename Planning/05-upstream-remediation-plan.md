...
Status: active plan; fork PR #1 merged the scrolling/configuration baseline, fork PR #2 carries the P0 compatibility series, `codex/remediation-p1` carries the first P1 architecture increments, fork PR #6 carries the generation-based scroll reset and helper-registration recovery work, `codex/remediation-login-items` carries the installed-path registration increment, `codex/remediation-policy-editor` carries the advanced app-selector authoring increment, `codex/remediation-lifecycle-closure` carries the current signed lifecycle verification, `codex/remediation-axis-direction-p1` carries the adapted axis-specific reverse-direction increment, and `codex/remediation-next-p1` carries the per-app policy runtime identity increment. This document supersedes `03-backlog-triage.md` and `04-roadmap.md` where they conflict.
...
The next per-app policy increment closes the runtime identity handoff. The scroll event tap now captures WindowServer's event-handler window number before dispatching to the worker queue, preferring `kCGMouseEventWindowUnderMousePointerThatCanHandleThisEvent` and falling back to the visual window field. The worker resolves that exact window through `CGWindowListCopyWindowInfo` with the required above-window option and explicit window-number filtering; missing metadata, malformed window entries, and non-positive owner PIDs fail closed. This removes AppKit hit-testing from the normal per-app policy path without changing the existing no-event compatibility API. Deterministic tests cover field precedence, missing IDs, non-first window entries, and missing window lists; the unsigned Xcode 27 Debug build and full local remediation suite pass. Physical Java/wrapper, full-screen, missing-app, app-switch, iPhone Mirroring, and signed-build checks remain gates.
