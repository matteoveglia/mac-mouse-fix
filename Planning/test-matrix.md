...
| Event-tap ownership core | Pass (deterministic) | Fake-backend tests cover failed tap/source creation, inert creation, idempotent toggles, refused enable, late-enable rejection, stable teardown order, and exactly-once release. All six live production owners now use the shared handle on their designated run loop. Signed runtime stress remains pending. |
