...
| Per-app policy authoring | Deterministic core pass; UI partial | The Scrolling tab now exposes an Advanced Application Rules editor for exact executable paths, wrapper bundle IDs/paths, and process names. It preserves bundle-ID rules, rejects duplicate/invalid selectors, and keeps process-name fallback constrained by the policy engine. Signed UI interaction and Java/wrapper/full-screen runtime behavior remain manual gates. |
